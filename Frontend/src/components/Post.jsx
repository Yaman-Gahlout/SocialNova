...
function Post({ post, profileData }) {
  ...
  async function commentHandler(postId) {
    try {
      ...
      const updatedPosts = profileData.posts.map((p) => {
  ...
  async function followHandler(targetUserId) {
    try {
      ...
      if (message === "User followed") {
        updatedFollowing = [...profileData.following, targetUserId];
        toast.success("User followed!");
      }

      if (message === "User unfollowed") {
        updatedFollowing = profileData.following.filter(
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-slate-800">
        ...
        <div className="pr-4">
          {isFollowing && userData?._id !== post?.author?._id && (
            <button
      ...
      {/* ===== MEDIA ===== */}
      <div className="h-100 w-[400px] bg-slate-900 flex items-center justify-center">
